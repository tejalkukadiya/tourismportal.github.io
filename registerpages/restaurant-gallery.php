...
<?php
	include '../Connection.php';
		...
		if ( isset($_GET["did"]))
		{
			$did=$_GET["did"];
			unlink($did);
			$delete="delete from restaurantgallery where img='$did'";
			$conn->query($delete);
		}
		session_start();
...
?>
	<form action="restaurant-gallery-entry.php" method="post" enctype="multipart/form-data" style="margin-top: 20px;margin-bottom: 50px;" />
...
<?php
	$result=$conn->query("select img from restaurantgallery where id=(select id from restaurant where username='$username')");
