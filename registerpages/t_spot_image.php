    ...
    $check = getimagesize($_FILES["fileToUpload"]["tmp_name"]);
            ...
            if ($uploadOk == 1) {
                if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                    ...
                    //echo $target_file;
                    $sql="insert into touristspotimages values('$spot','$target_file')";
                     echo $sql;                     
            } else {
                        $uploadOk = 2;
      ...
      if ($uploadOk != 1) {
        ...
        header("Location:t_spot_image_display.php?e=$uploadOk");
        return;    
        // if everything is ok, try to upload file
    }
    $conn->query($sql);
    header("Location:t_spot_image_display.php?tid=$spot");
?>
