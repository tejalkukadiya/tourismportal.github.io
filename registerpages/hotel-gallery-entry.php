...
$target_dir = "uploadss/hotels/";
$target_file = $target_dir.$time. basename($_FILES["fileToUpload"]["name"]);
$uploadOk = 1;
$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);

if(isset($_POST["submit"])) {
$result=$conn->query("select id from hotel where username='$username'");
    ...
    $check = getimagesize($_FILES["fileToUpload"]["tmp_name"]);
            ...
            if ($uploadOk == 1) {
                if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                    $sql="insert into hotelgallery values('$id','$target_file')";
                      echo $sql;                
            } else {
                        $uploadOk = 2;
      ...
      if ($uploadOk != 1) {
        ...
        header("Location:hotel-gallery.php?e=$uploadOk");
        return;    
        // if everything is ok, try to upload file
    }
    if($conn->query($sql)===TRUE) echo 'done' ;
    else echo 'not done';
    //header("Location:hotel-gallery.php");
}
?>
